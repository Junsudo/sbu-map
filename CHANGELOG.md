...
## 2026-08-20 — 3차 감사 수리
- 터치 기기에서 전 지역 업소 점과 내 핀이 탭되지 않던 문제 수정(터치 핸들러 분기 누락).
- 세로 화면에서 업소 점이 22배 이하에서 나타나지 않던 문제 수정(가시 범위 계산을 맵 경계로 클램프).
- 소형 건물 지연 빌드가 865ms 단일 블록이던 것을 12만 개 단위 슬라이스로 분할하고, 빌드 후 원본 좌표 배열을 해제해 메모리를 줄임.
- 근접 퍼지가 다른 이름의 정상 업소 약 190곳(Disney Store, Station Pizza 등)을 지우던 것을 이름 기반 dedupe로 교체해 복원. 투영 범위 밖 1,254곳과 주소형 노이즈 제거. 최종 76,085곳.
- 자정 넘는 열차(24:17, 24:26)가 목록 맨 위 00:17로 정렬되던 것을 맨 뒤로 수정.
- OSM 유래 문자열 전부에 HTML escape 적용(현재 데이터엔 공격 문자열 없음, 갱신 대비 방어).
- service worker precache가 브라우저 HTTP 캐시를 거치며 갱신 직후 낡은 데이터를 저장할 수 있던 것을 cache:'reload'로 수정(v5).
- 셀 경계 클릭 시 인접 셀의 더 가까운 점을 못 잡던 것 보정(3×3 탐색), 라벨이 점 클릭을 가로채던 것 수정, Suffolk 51 라벨을 경로 위로 이동, 오염된 업소명 1건 정정, 스플래시가 오류 시 안 사라지던 경로 보강.
